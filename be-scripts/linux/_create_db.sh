...
for i in {1..60}; do
  if docker run --rm --network container:mssql "$TOOLS_IMAGE" /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P "$SA_PASSWORD" -Q 'SELECT 1' >/dev/null 2>&1; then
...
docker run --rm --network container:mssql "$TOOLS_IMAGE" /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P "$SA_PASSWORD" -Q "$CREATE_SQL"
