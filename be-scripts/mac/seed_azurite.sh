...
#
# Notes:
# - If EMULATOR_SAS_* are not provided and Azure CLI is installed, we will generate SAS tokens
#   for `devstoreaccount1` using the well-known Azurite development key.
# - If Azure CLI is not installed and EMULATOR_SAS_* are missing, the script will exit with instructions.
...
maybe_warn_container() {
  if command -v docker >/dev/null 2>&1; then
    if ! docker ps --format '{{.Names}}' | grep -q '^azurite$'; then
      echo "WARN: Azurite container 'azurite' not detected. Ensure it's running (be-scripts/mac/start.sh)." >&2
    fi
  fi
}
...
ensure_emulator_sas() {
  if command -v az >/dev/null 2>&1; then
    ...
    for v in EMULATOR_SAS_CONTENT EMULATOR_SAS_VIDEO_PRIVATE EMULATOR_SAS_VIDEO_PUBLIC; do
      if [ -z "${(P)v:-}" ]; then
        echo "ERROR: $v not set and Azure CLI 'az' not available to auto-generate. Export these SAS tokens (no leading '?')." >&2
        exit 1
      fi
    done
  fi
